...
# Define file paths
csv_file_path = 'detection_YOLOv8_descending.csv'
...
while True:
    ...
    if frame_count % int(cap.get(cv2.CAP_PROP_FPS) / frame_rate) == 0:
        ...
        # Filter out detections with confidence score < 0.5
        detections_to_save = [det for det in detections_to_save if det[7] >= 0.5]
        
        # Sort detections in descending order of confidence score
        detections_to_save = sorted(detections_to_save, key=lambda x: x[7], reverse=True)
