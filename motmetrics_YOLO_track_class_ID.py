...
pred_txt_path = '/work/cshah/updatedYOLOv8/ultralytics/New_filtered_track_results_yolo_mot.txt'
...
# Ensure frame indices are properly handled
gt_frame_index = gt.index.get_level_values(0).astype(int) if isinstance(gt.index, pd.MultiIndex) else gt.index.astype(int)
pred_frame_index = pred.index.get_level_values(0).astype(int) if isinstance(pred.index, pd.MultiIndex) else pred.index.astype(int)

# Extract and map class IDs
...
# Define class ID mapping (adjust according to your dataset)
class_id_mapping = {
...
for frame in all_frames:
    ...
    # Compute IoU matrix if detections are present
