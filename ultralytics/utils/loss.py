...
class CustomIoULoss(nn.Module):
    ...
    def forward(self, pred_bboxes, target_bboxes, target_scores, fg_mask):
        # Compute IoU
        ...
        # Ensure fg_mask and iou are compatible
        if fg_mask.dim() == 2 and iou.dim() == 3:
            fg_mask = fg_mask.view(batch_size, num_boxes)  # Reshape to [batch_size, num_boxes]
            iou = iou.view(batch_size, num_boxes)  # Reshape to [batch_size, num_boxes]
        else:
            raise ValueError(f"Unexpected fg_mask dimensions: {fg_mask.dim()} or IoU dimensions: {iou.dim()}")

        # Apply mask to IoU and target_scores
        iou_masked = iou[fg_mask]  # Apply mask
        target_scores_flat = target_scores.view(batch_size, num_boxes)[fg_mask]  # Flatten target_scores

        if iou_masked.size(0) != target_scores_flat.size(0):
            raise ValueError(f"Size mismatch: IoU tensor size {iou_masked.size()} does not match weight tensor size {target_scores_flat.size()}")

        # Compute loss
        loss_iou = ((1.0 - iou_masked) * target_scores_flat).sum() / target_scores_flat.sum()  # Normalize by sum of weights
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        #loss_iou = wise_iou_loss_value + loss_iou

        # Combine the two loss values (average)
        #combined_loss_average = (loss_iou + wise_iou_loss_value) / 2
        #loss_iou = combined_loss_average

        #combined_loss_min = torch.min(loss_ciou, wise_iou_loss_value)
        #loss_iou = combined_loss_min

        #combined_loss_max = torch.max(loss_ciou, wise_iou_loss_value)
        #loss_iou = combined_loss_max

        combined_loss_geometric_mean = torch.sqrt(loss_ciou * wise_iou_loss_value)
        loss_iou = combined_loss_geometric_mean

        ## Combine the two loss values (sum)
        #combined_loss_sum = loss_iou + wise_iou_loss_value
